...
Built-in annotations like `@NotBlank` and `@Email` only cover simple, static field checks. When a rule needs actual logic — checking membership in a business-specific set, comparing two fields, or calling a lookup — write a custom constraint instead of stuffing that logic into the service layer as an afterthought:
...
Treat `null` as valid inside the custom validator and pair it with a separate `@NotNull` when the field is actually required — that keeps "is it present" and "is it one of the allowed values" as two independent, composable rules. For a check that spans multiple fields (e.g. `endDate` must be after `startDate`), put `@Constraint` on the class itself instead of a single field, and implement `isValid` against the whole object.
...
**Answer:** When the rule needs actual logic beyond a static pattern — checking membership in a dynamic or business-specific set, comparing two fields against each other, or calling a lookup. Implement `ConstraintValidator`, treat `null` as valid inside it and pair it with a separate `@NotNull` for "required," and use a class-level constraint instead of a field-level one when the rule spans multiple fields.
